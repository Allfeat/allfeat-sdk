...
//! including their identification, metadata, participants, and classification.
//!
//! # Core Types
//!
//! - [`MusicalWork`] - The main structure representing a musical composition
//! - [`Iswc`] - International Standard Musical Work Code identifier
//! - [`MusicalWorkType`] - Classification of work types (original, medley, mashup, adaptation)
//! - [`Participant`] - Contributors to the musical work
//! - [`ParticipantRole`] - Roles that participants can have
...
/// - `participants` - List of people and entities involved in creating the work
...
/// - `participants: Vec<Participant>` → `participants: BoundedVec<Participant, ConstU32<256>>`
///
/// # Examples
///
/// ## Simple Song
...
///     iswc: Iswc("T-987654321-0".to_string()),
...
///
/// # Type Transformation
/// In runtime mode: `String` → `BoundedVec<u8, ConstU32<11>>`
#[runtime_midds]
pub struct Iswc(
    /// The ISWC string, limited to 11 characters in runtime mode.
    #[runtime_bound(11)]
    pub String,
);
...
///
/// Participants represent individuals or entities involved in the creative
...
/// # Examples
/// ```rust
/// # use allfeat_midds_v2::musical_work::{Participant, ParticipantRole};
/// let composer = Participant {
///     id: 12345,
///     role: ParticipantRole::Composer,
/// };
///
/// let lyricist = Participant {
///     id: 67890,
...
pub struct Participant {
    ...
    pub id: MiddsId,
    /// The specific role this participant played in the creation of the work.
    pub role: ParticipantRole,
}

/// Roles that participants can have in the creation of a musical work.
///
...
/// # Examples
/// ```rust
/// # use allfeat_midds_v2::musical_work::ParticipantRole;
/// let roles = vec![
///     ParticipantRole::Composer,   // Created the music
///     ParticipantRole::Author,     // Wrote the lyrics
///     ParticipantRole::Arranger,   // Created orchestral arrangement
///     ParticipantRole::Publisher,  // Manages publication rights
/// ];
/// ```
#[runtime_midds]
...
#[cfg(all(feature = "runtime", feature = "runtime-benchmarks"))]
impl BenchmarkHelper<MusicalWork> for MusicalWork {
    fn benchmark_instance(i: u32) -> MusicalWork {
        MusicalWork {
            iswc: Iswc(create_bounded_string::<11>(i)),
            ...
            work_type: if i == 0 {
                ...
                Some(MusicalWorkType::Medley(create_bounded_vec::<MiddsId, 512>(
                    42u64, i,
                )))
            } else if i % 3 == 1 {
                Some(MusicalWorkType::Mashup(create_bounded_vec::<MiddsId, 512>(
                    42u64, i,
                )))
            } else {
                Some(MusicalWorkType::Original)
            },
            participants: create_bounded_vec::<Participant, 256>(
                Participant {
                    id: 42u64,
                    role: ParticipantRole::Composer,
                },
